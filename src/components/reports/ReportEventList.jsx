...
const EventItem = ({ event, client, work, onClick }) => {
  ...
  return (
    <div
      className="flex items-center gap-4 px-2 py-3 hover:bg-slate-800/50 rounded-lg transition-colors cursor-pointer"
      onClick={() => onClick(event)}
    >
      <Avatar className="h-10 w-10 border-2 border-slate-700 hidden sm:flex">
      ...
      <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
        <div className="col-span-2 md:col-span-1">
          ...
          <p className="text-sm text-slate-400 truncate">{client?.name || 'Cliente não encontrado'}</p>
        </div>
