...
const statConfigs = [
  { key: 'faturamento_pago', label: 'Recebido', icon: '✅', route: '/reports', hint: 'Ver relatórios', format: (v) => `R$${v.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}` },
        ...
        >
          <NeonGlass primary={primaryHex} accent={accentHex} className="p-4 relative">
            ...
            {isLoading ? <div className="h-6 bg-[#1a1d27] rounded animate-pulse" /> : (
              <p className="text-lg font-extrabold text-white leading-tight" style={{ textShadow: `0 0 20px ${primaryHex}33` }}>{config.format(stats[config.key] ?? 0)}</p>
            )}
